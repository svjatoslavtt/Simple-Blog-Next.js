...
function* getPosts() {
  try {
		const response: any = yield call(axios.get, process.env.REACT_APP_API + ApiEndPoints.POSTS);
...
function* getCurrentPost(action: any) {
  try {
		const response: any = yield call(axios.get, 'https://simple-blog-api.crew.red' + ApiEndPoints.POSTS + '/' + action.payload.postId + '?_embed=comments');	
...
function* addComment(action: any) {
  try {
		const response: any = yield call(axios.post, 'https://simple-blog-api.crew.red' + ApiEndPoints.ADD_COMMENT, action.payload);	
...
function* addNewBlog(action: any) {
  try {
		...
		const response: any = yield call(axios.post, 'https://simple-blog-api.crew.red' + ApiEndPoints.POSTS, rest);	
...
function* updatePost(action: any) {
  try {
		const response: any = yield call(axios.put, 'https://simple-blog-api.crew.red' + ApiEndPoints.POSTS + '/' + action.payload.id, action.payload);	
...
function* deletePost(action: any) {
  try {
		...
		const response: any = yield call(axios.delete, 'https://simple-blog-api.crew.red' + ApiEndPoints.POSTS + '/' + postId);	
